...
function* aesGen() {
  ...
  yield S(() => { status.textContent = 'AES 分组密码：16 字节一块；4 层变换 = 非线性（S 盒）+ 扩散（ShiftRows/MixColumns）+ 混淆（轮密钥）。初始状态 = 明文 ' + hx(PT) + '，密钥 ' + hx(KEY); phaseT.setText('初始状态'); });
  yield W(900);
  renderState(state);
  allColor(BLUE);
  yield S(() => { status.textContent = '第 0 轮（预白化）：AddRoundKey —— 明文 ⊕ 初始轮密钥'; phaseT.setText('AddRoundKey(0)'); });
  yield W(650);
  doKey(state, 0);
  renderState(state);
  yield W(650);
  for (let round = 1; round <= 10; round++) {
    if (round === 1) {
      yield S(() => { status.textContent = '第 1 轮 ① SubBytes：每字节经 S 盒非线性替换（GF(2⁸) 求逆 + 仿射）'; phaseT.setText('SubBytes'); });
      yield W(700);
      for (let r = 0; r < 4; r++) { ROW[r].forEach(i => stateChips[i].setColor(ORANGE, ORANGE)); yield W(240); }
      allColor(BLUE);
      doSub(state); renderState(state);
      yield S(() => { status.textContent = '第 1 轮 ② ShiftRows：第 r 行循环左移 r 格 —— 列间扩散'; phaseT.setText('ShiftRows'); });
      yield W(700);
      for (let r = 0; r < 4; r++) { ROW[r].forEach(i => stateChips[i].setColor(CYAN, CYAN)); yield W(240); }
      allColor(BLUE);
      doShift(state); renderState(state);
      yield S(() => { status.textContent = '第 1 轮 ③ MixColumns：每列与固定矩阵做 GF(2⁸) 乘法 —— 字节间扩散'; phaseT.setText('MixColumns'); });
      yield W(700);
      for (let c = 0; c < 4; c++) { COL[c].forEach(i => stateChips[i].setColor(PUR, PUR)); yield W(240); }
      allColor(BLUE);
      doMix(state); renderState(state);
      yield S(() => { status.textContent = '第 1 轮 ④ AddRoundKey：⊕ 轮密钥 1（' + hx(KEY_EXP.slice(16, 32)) + '）'; phaseT.setText('AddRoundKey'); });
      yield W(700);
      doKey(state, 1); renderState(state);
      allColor(GOLD); yield W(450);
    } else if (round === 10) {
      yield S(() => { status.textContent = '第 10 轮（末轮）：无 MixColumns —— SubBytes → ShiftRows → AddRoundKey'; phaseT.setText('Round 10'); });
      yield W(550);
      doSub(state); renderState(state);
      ...
      yield S(() => { status.textContent = '第 ' + round + ' 轮：SubBytes → ShiftRows → MixColumns → AddRoundKey（快进，真实运算）'; phaseT.setText('Round ' + round + '/10'); });
      yield W(240);
  ...
  yield W(1000);
  yield S(() => { status.textContent = 'AES 演示完成：10 轮 × 4 层变换 = 现代对称加密的标准答案，时间复杂度 O(轮数 × 块大小)'; phaseT.setText('等待运行'); });
  yield W(400);
}
