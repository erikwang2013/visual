// AlgorithmLibrary/ActivitySelect3D.js — 活动选择（贪心）：按结束时间排序，每次选最早结束且兼容的活动
...
const hint = new VText(scene, { text: '点击「运行活动选择」开始', x: 0, y: 300, z: 0, color: PALETTE.textGlow, scale: 0.85 });
...
function runActivity() {
  resetAll();
  hint.setText('贪心 ≠ 碰运气：要证明「选最早结束」的最优性 —— 任何最优解都能把第一个活动换成它（交换论证）');
  C(700, () => {
  ...
  const fin = acSteps[acSteps.length - 1];
  C(1000, () => {
    fin.chosen.forEach(id => { bars[id - 1].setColor(GREEN, GREEN); bars[id - 1].setText('✓ 选'); });
    totalT.setText('最多可选 ' + fin.chosen.length + ' 个：' + fin.chosen.map(i => 'A' + i).join('、'));
    stageT.setText('扫描完 11 个活动 → 最多 ' + fin.chosen.length + ' 个：A1（1-4）、A4（5-7）、A8（8-11）、A11（12-16）');
    hint.setText('为什么不是 A2/A3/A6？它们开始太早、和已选重叠；结束最早的兼容活动永远不差');
  });
  C(1100, () => {
    outT.setText('最多 ' + fin.chosen.length + ' 个活动 —— 若按「开始最早」贪心：A3（0-6）→ A4 → A8 → A11 也是 4 个；但「最短时长」贪心会输，只能选 2 个');
    status.textContent = '活动选择最多 ' + fin.chosen.length + ' 个（A1,A4,A8,A11）';
...
panel.addButton('运行活动选择', runActivity);
