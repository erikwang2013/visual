// AlgorithmLibrary/ArithmeticCoding3D.js — 算术编码：区间不断缩窄，输出区间内一个小数
...
const hint = new VText(scene, { text: '点击「运行编码」开始', x: 0, y: 300, z: 0, color: PALETTE.textGlow, scale: 0.85 });
...
const marker = new VArrow(scene, { x: -BAR / 2, y: 30, z: 0, down: true });
...
function runEncode() {
  resetAll();
  hint.setText('算术编码：把整条消息映射到 [0,1) 内的一个区间，每读一个符号区间就按概率缩窄');
  let done = 0;
  const next = () => {
    if (done >= MSG.length) {
      const code = F.toFixed(4);
      codeText.setText('编码：区间中点 ' + F.toFixed(4) + '，需 ' + bits + ' bit（H-L = ' + (H[MSG.length] - L[MSG.length]).toFixed(4) + ' < 2⁻' + (bits - 1) + '）');
      status.textContent = MSG + ' → ' + code + '（' + bits + ' bit，理论极限 ≈ ' + bits + '）';
    ...
    const x = (nL + nH) / 2;
    marker.moveTo((x - 0.5) * BAR, 30, 0, 450);
    rangeText.setText('区间 [' + l.toFixed(4) + ', ' + h.toFixed(4) + ')，读到「' + ch + '」');
    C(150, () => {
      placeSegs(l, h);
      for (const k of ['A', 'B', 'C']) {
        segs[k].mesh.material.opacity = k === ch ? 1 : 0.3;
      }
    });
    hint.setText('P(' + ch + ') = ' + PROB[ch] + ' → 新区间 = [' + l.toFixed(3) + ', ' + h.toFixed(3) + ') × ' + PROB[ch] + ' = [' + nL.toFixed(4) + ', ' + nH.toFixed(4) + ')');
    C(950, () => {
      outText.setText('已读：' + MSG.slice(0, done) + ' → 当前区间 [' + nL.toFixed(4) + ', ' + nH.toFixed(4) + ')');
...
panel.addLabel('（拖拽旋转视角，滚轮缩放；JPEG 2000 / xz 用它逼近熵极限）');
