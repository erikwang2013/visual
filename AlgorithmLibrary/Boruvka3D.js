...
function* boruvkaGen() {
  ...
  colorComponents();
  yield S(() => { status.textContent = '初始：每个点自成一个分量。每轮各分量选「最廉价跨分量出边」并合并'; });
  yield W(700);
  let total = 0, round = 0;
  while (true) {
    const comps = new Map();
    ...
    round++;
    yield S(() => { status.textContent = '第 ' + round + ' 轮：' + comps.size + ' 个分量，各选最小出边'; });
    yield W(500);
    ...
    for (const [root, c] of comps) {
      ...
      yield S(() => { status.textContent = '分量 {' + c.nodes.join(',') + '} 最小出边：' + c.min[0] + '-' + c.min[1] + '（' + c.min[2] + '）'; });
      yield W(380);
    }
    yield S(() => { status.textContent = '合并：本轮选出的边全部加入 MST'; });
    yield W(350);
    for (const [root, c] of comps) {
      const [u, v, w] = c.min;
    ...
    colorComponents();
    yield S(() => { status.textContent = '第 ' + round + ' 轮后 MST 边数 ' + mstEdges.length + '，总权 ' + total + '；parent [' + parent.join(', ') + ']'; });
    yield W(500);
    resetEdgeColors();
  }
  yield S(() => { status.textContent = '只剩一个分量 → 合并完成。MST 边集：' + mstEdges.map(e => e[0] + '-' + e[1]).join('  '); });
  yield W(400);
  for (const [u, v] of mstEdges) { setEdgeColor(u, v, GOLD, 1); yield W(200); }
  yield S(() => { status.textContent = 'Boruvka 演示完成：MST 总权 ' + total + '，共 ' + round + ' 轮，O(E·logV)'; });
  yield W(600);
  resetEdgeColors();
...
function* runBoruvka() {
  buildGraph();
  yield S(() => { status.textContent = 'Boruvka：每轮每分量取最小出边合并，轮数 ≤ logV'; });
  yield W(400);
  yield* boruvkaGen();
}
