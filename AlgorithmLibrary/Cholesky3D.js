// AlgorithmLibrary/Cholesky3D.js — Cholesky 分解：对称正定阵 A = L·Lᵀ —— 取「带平方根的下三角」，比 LU 少一半乘除法
...
const hint = new VText(scene, { text: '点击「运行 Cholesky 分解」开始', x: 0, y: 300, z: 0, color: PALETTE.textGlow, scale: 0.85 });
...
function runCholesky() {
  resetAll();
  hint.setText('核心：A 的第 (i,j) 格 = L 第 i 行与 L 第 j 行的点积 —— 所以 L[i][j] 可从 A[i][j] 反推，对角元先就位');
  steps.forEach((s, t) => {
    const refs = [];
    s.terms.forEach(({ k }) => {
    ...
    C(600, () => {
      ...
      stageT.setText(`填 L[${s.i}][${s.j}]：从 A[${s.i}][${s.j}] = ${A0[s.i][s.j]} 出发，扣掉已填项的贡献`);
      hint.setText('自左上往右下填：每个新格只依赖「它左上方已就位的格」—— 行与列同步推进');
    });
    C(700, () => {
      ...
      if (s.diag) {
        eqT.setText(`s = ${A0[s.i][s.j]}${s.terms.map(({ k }) => ` − L[${s.i}][${k}]²`).join('')} = ${s.s} → L[${s.i}][${s.j}] = √${s.s} = ${v}`, { color: GOLD });
        stageT.setText(`对角元：扣掉 ΣL² 后开平方根 —— 正定性保证根号内恒为正`);
        hint.setText('为什么能开方？正定阵的所有主子式 > 0，每一步根号下的数都严格大于 0 —— 分解永不失败');
      } else {
        eqT.setText(`s = ${A0[s.i][s.j]}${s.terms.map(({ k }) => ` − L[${s.i}][${k}]·L[${s.j}][${k}]`).join('')} = ${s.s} → L[${s.i}][${s.j}] = s / L[${s.j}][${s.j}] = ${v}`, { color: AMBER });
        stageT.setText(`非对角元：扣掉交叉积后除以 L[${s.j}][${s.j}] —— 玫瑰色 = 被引用的已填项`);
      }
    });
    C(320, () => {
  ...
  C(1000, () => {
    outT.setText(`L = [[2,0,0],[6,1,0],[−8,5,3]] —— 验证 L·Lᵀ = A：第 1 行 2×(2,6,−8) = (4,12,−16) ✓，第 2 行 6×(2,6,−8)+1×(0,1,5) = (12,37,−43) ✓`);
    status.textContent = `Cholesky 分解：L = [[2,0,0],[6,1,0],[−8,5,3]]（L·Lᵀ = A 验证通过）`;
    hint.setText('用途：解 Ax=b 只需一次分解、两次三角回代 —— 比 LU 快一倍，且 L 直接给出「平方根协方差」矩阵');
  });
  C(1200, () => {
    outT.setText('复杂度 O(n³/6)：LU 的 O(n³/3) 的一半。应用：高斯过程、Kalman 滤波、AᵀA 正规方程、MCMC 采样');
    hint.setText('变体 LDLᵀ 免开方（对角为 d）；要求对称 + 正定 —— 数值分析里最优雅的「免费午餐」');
  });
}

panel.addButton('运行 Cholesky 分解', runCholesky);
