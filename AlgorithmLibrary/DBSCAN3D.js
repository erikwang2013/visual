// AlgorithmLibrary/DBSCAN3D.js — DBSCAN：密度聚类，自动发现簇与噪声
...
const hint = new VText(scene, { text: '点击「密度聚类」开始', x: 0, y: 255, z: 0, color: PALETTE.textGlow, scale: 0.85 });
...
function lightCluster(idx, color) {
  idx.forEach(i => pts[i].setColor(color, color));
}

function runDBSCAN() {
  resetAll();
  hint.setText('DBSCAN：邻居多 → 核心点，核心点串起来成簇；没人要的点就是噪声');
  C(300, () => { stepT.setText('第 1 步：随机选点，数它 ε=1.1 半径内的邻居数（≥ 3 才是核心点）'); });
  C(700, () => {
...
panel.addButton('密度聚类', runDBSCAN);
