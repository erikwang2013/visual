...
const hint = new VText(scene, { text: '点击「运行演示」开始：DFS 从节点 0 出发', x: 0, y: 300, z: 0, color: PALETTE.textGlow, scale: 0.85 });
...
// ---- 栈可视化（顶部一排盒，栈顶在右） ----
function* pushBox(id) {
  const x = -190 + stackBoxes.length * 55;
...
panel.addButton('运行演示', () => engine.start(runDFS()));
