...
const P = (x, y) => ({ x, y, z: 0 });
// 6 条边模块级预建（3 持有蓝 + 3 请求红），运行时仅显隐/变色，generator 内不 new
const edges = new Map();
for (let i = 0; i < 3; i++) {
  const hold = tubeBetween(scene, P(PPOS[i][0], PPOS[i][1]), P(RPOS[i][0], RPOS[i][1]), { color: BLUE, opacity: 0.8, radius: 4 });
  hold.visible = false;
  edges.set('hold' + i, hold);
  const req = tubeBetween(scene, P(RPOS[(i + 1) % 3][0], RPOS[(i + 1) % 3][1]), P(PPOS[i][0], PPOS[i][1]), { color: RED, opacity: 0.8, radius: 3.5 });
  req.visible = false;
  edges.set('req' + i, req);
}
...
function* deadlockGen() {
  ...
  yield W(800);
  for (let i = 0; i < 3; i++) {
    yield S(() => { edges.get('hold' + i).visible = true; status.textContent = 'P' + i + ' 持有 R' + i + '（蓝边）—— 资源不共享、进程不放手'; });
    yield W(650);
  }
  ...
  for (let i = 0; i < 3; i++) {
    const req = (i + 1) % 3;
    yield S(() => { edges.get('req' + i).visible = true; status.textContent = 'P' + i + ' 请求 R' + req + '（红边）—— 被 P' + req + ' 持有；条件③④：不可剥夺 + 循环等待'; });
    yield W(650);
  }
