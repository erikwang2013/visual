...
const hint = new VText(scene, { text: '点击「▶ 演示」开始：Diffie-Hellman —— 公开信道交换，双方算出同一个只有彼此知道的密钥', x: 700, y: 560, z: 0, color: PALETTE.textGlow, scale: 0.7, wrapChars: 7 });
const status = panel.addStatus('就绪');
const stageT = new VText(scene, { text: '', x: 320, y: 555, z: 0, color: GOLD, scale: 0.72 });
const eqT = new VText(scene, { text: '', x: 700, y: 310, z: 0, color: PALETTE.textGlow, scale: 0.44, wrapChars: 8 });
...
new VText(scene, { text: '公开参数：素数 p = ' + P + '，原根 g = ' + G + '（全信道公开，谁都能听）', x: 700, y: 640, z: 0, color: PALETTE.textDim, scale: 0.55, wrapChars: 8 });
const alice = new VNode(scene, { x: 60, y: 335, z: 0, radius: 30, label: 'Alice', color: 0xa78bfa, emissive: 0xa78bfa });
const bob = new VNode(scene, { x: 660, y: 335, z: 0, radius: 30, label: 'Bob', color: 0x38bdf8, emissive: 0x38bdf8 });
const lineTube = tubeBetween(scene, alice.mesh.position, bob.mesh.position, { color: PALETTE.edge, opacity: 0.25 });

const aT = new VText(scene, { text: '', x: 60, y: 500, z: 0, color: RED, scale: 0.7 });
const bT = new VText(scene, { text: '', x: 660, y: 500, z: 0, color: RED, scale: 0.7 });
const AT = new VText(scene, { text: '', x: 60, y: 425, z: 0, color: BLUE, scale: 0.75 });
const BT = new VText(scene, { text: '', x: 660, y: 425, z: 0, color: BLUE, scale: 0.75 });
const sA = new VText(scene, { text: '', x: 60, y: 265, z: 0, color: GREEN, scale: 0.75 });
const sB = new VText(scene, { text: '', x: 660, y: 265, z: 0, color: GREEN, scale: 0.75 });
const sharedNode = new VNode(scene, { x: 360, y: 235, z: 0, radius: 30, label: '共享密钥', color: DIM, emissive: DIM });
...
function* dhGen() {
  ...
  yield W(850);
  yield A(750, () => { AT.moveTo(660, 425, 0, 700); BT.moveTo(60, 425, 0, 700); });
...
panel.addButton('清空', () => {
  ...
  AT.moveTo(60, 425, 0, 1); BT.moveTo(660, 425, 0, 1);
