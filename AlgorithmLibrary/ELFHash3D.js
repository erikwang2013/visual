// AlgorithmLibrary/ELFHash3D.js — ELF 哈希：h = (h<<4) + 字符码，溢出时把高 4 位异或回低 8 位并清零 —— Unix 可执行文件的哈希
...
const hint = new VText(scene, { text: '点击「运行 ELF 哈希」开始', x: 0, y: 300, z: 0, color: PALETTE.textGlow, scale: 0.85 });
...
function runELF() {
  resetAll();
  hint.setText('ELF 的思路 = 移位混合：每字符 h <<= 4 再加，溢出位不扔掉 —— 高 4 位异或回低 8 位，等于让每个字符参与两次混合');
  elfSteps.forEach((s, t) => {
    C(560, () => {
      const bx = charBoxes[t];
      bx.setColor(CYAN, CYAN);
...
panel.addButton('运行 ELF 哈希', runELF);
