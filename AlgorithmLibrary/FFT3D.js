...
const hint = new VText(scene, { text: '点击「运行演示」开始：FFT 4 点变换', x: 0, y: 300, z: 0, color: PALETTE.textGlow, scale: 0.85 });
...
function* fftGen() {
  yield S(() => { hint.setText('朴素求值：在 n 个点上每个点做 n 次乘加 = O(n²)。FFT 的戏法：ω 的幂有对称性 —— ω^(k+n/2) = −ω^k'); stageT.setText('输入 a = [1, 2, 3, 4]（n = 4）—— 目标：在 ω⁰, ω¹, ω², ω³ 四个点上求多项式值'); });
  yield W(700);
  for (let i = 0; i < 4; i++) inBoxes[i].setColor(CYAN, CYAN);
  yield S(() => hint.setText('X(k) = Σ a_j·ω^(jk)。直接算 16 次乘法 —— 但 ω 的周期性和对称性可以省一半以上'));
  yield W(600);
  yield S(() => { stageT.setText('分治：按下标奇偶拆分 —— 偶部 a0,a2 = [1,3]，奇部 a1,a3 = [2,4]'); eqT.setText('关键代数：X(k) = E(k²) + ω^k·O(k²)，X(k+2) = E(k²) − ω^k·O(k²)', { color: PALETTE.textGlow }); });
  yield W(750);
  yield S(() => hint.setText('两个「半个」的 FFT 就能拼出整个：一半加 ω^k 的倍，一半减 —— 这就是蝶形的「翅膀」'));
  yield W(300);
  E.forEach((v, i) => setCell(eBoxes[i], v, VIOLET));
  eqT.setText('偶部 2 点 FFT：[1+3, 1−3] = [4, −2]', { color: VIOLET });
  yield S(() => stageT.setText('2 点 FFT 就是一次加减：X(0) = 1+3 = 4，X(1) = 1−3 = −2'));
  yield W(750);
  O.forEach((v, i) => setCell(oBoxes[i], v, AMBER));
  eqT.setText('奇部 2 点 FFT：[2+4, 2−4] = [6, −2]', { color: AMBER });
  yield S(() => stageT.setText('同款操作：X(0) = 2+4 = 6，X(1) = 2−4 = −2 —— 递归到这里深度为 0，直接算'));
  yield W(750);
  setCell(outBoxes[0], out[0], GOLD);
  setCell(outBoxes[2], out[2], GOLD);
  eqT.setText(bt1[0] + '；' + bt1[1], { color: GOLD });
  yield S(() => { stageT.setText('蝶形对 1：E0 与 O0 用 ω⁰ = 1 —— X0 = E0+O0，X2 = E0−O0（ω² = −1 使减号免费）'); hint.setText('ω² = −1：X2 和 X0 共用一次计算 —— 这就是「一半计算变零计算」的数学出处'); });
  yield W(800);
  setCell(outBoxes[1], out[1], GOLD);
  setCell(outBoxes[3], out[3], GOLD);
  eqT.setText(bt2[0] + '；' + bt2[1], { color: GOLD });
  yield S(() => { stageT.setText('蝶形对 2：E1 与 O1 用 ω¹ = i —— X1 = E1+i·O1，X3 = E1−i·O1'); hint.setText('i² = −1 再次帮忙：乘一次 i，加出 X1 减出 X3 —— 一个蝶形出两个结果'); });
  yield W(800);
  yield S(() => { outT.setText('FFT(a) = [10, −2−2i, −2, −2+2i] ✓ —— 4 点只做了 4 个蝶形共 8 次乘加（朴素 16 次）'); status.textContent = 'FFT：[1,2,3,4] → [10, −2−2i, −2, −2+2i]（蝶形 4 个，乘法减半）'; });
  yield W(1000);
  yield S(() => hint.setText('验证：X(0) = 1+2+3+4 = 10 —— 直流分量。逆变换乘 1/n 再跑一遍 FFT 即还原'));
  yield W(400);
  yield S(() => { hint.setText('FFT 完成：4 点变换得 [10, −2−2i, −2, −2+2i]'); outT.setText('复杂度 T(n) = 2T(n/2) + O(n) → O(n log n)。应用：卷积（多项式乘法）、信号频谱、大整数乘法（FFT×FFT 分治）'); });
  yield W(1200);
  yield S(() => { hint.setText('把卷积的两边 FFT → 逐点乘 → 逆 FFT：O(n log n) 顶替 O(n²) —— 音频均衡器每天都在跑它'); outT.setText(''); });
  yield W(800);
}
...
panel.addButton('运行演示', () => engine.start(runFFT()));
