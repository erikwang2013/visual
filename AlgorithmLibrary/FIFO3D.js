...
const hint = new VText(scene, { text: '点击「▶ 演示」开始：FIFO —— 先装进来的页面先被换出', x: 700, y: 560, z: 0, color: PALETTE.textGlow, scale: 0.7, wrapChars: 7 });
const status = panel.addStatus('就绪');
const stageT = new VText(scene, { text: '', x: 320, y: 555, z: 0, color: GOLD, scale: 0.72, wrapChars: 7 });
const eqT = new VText(scene, { text: '', x: 700, y: 370, z: 0, color: PALETTE.textGlow, scale: 0.5, wrapChars: 8 });
...
engine.queue(() => runFIFO());
panel.addButton('清空', () => { engine.clear(); stageT.setText(''); eqT.setText(''); outT.setText(''); slots = [-1, -1, -1]; chips.forEach(c => c.setColor(DIM, DIM)); frames.forEach((f, i) => { f.setText('空'); f.setColor(BLUE, BLUE); f.moveTo(FRAME_X[i], 300, 0, 10); }); hint.setText('已清空，可重新运行'); status.textContent = ''; });
