  ...
  while (stack.length) {
    ...
    for (let i = 0; i < E.length; i++) {
      ...
      if (a === u && flow[i] < c && !visited.has(b)) {
        ...
        yield S(() => { status.textContent = '正向残余 ' + NAME[a] + '→' + NAME[b] + '（余量 ' + (c - flow[i]) + '）'; });
        yield W(280);
      }
      if (b === u && flow[i] > 0 && !visited.has(a)) {
        visited.add(a); parent[a] = i; dir[a] = -1; resC[a] = flow[i]; stack.push(a);
        yield S(() => { status.textContent = '反向残余 ' + NAME[b] + '→' + NAME[a] + '（退流量 ' + flow[i] + '）'; });
        yield W(280);
      }
    }
  }
...
function* ffGen() {
  ...
  buildGraph();
  refreshResidual();
  yield S(() => { status.textContent = '初始：所有边流量 0。在残余网络中 DFS 找 s→t 增广路（紫色细线 = 反向残余边）'; });
  yield W(600);
  let total = 0, aug = 0;
  while (true) {
    aug++;
    yield S(() => { status.textContent = '——— 寻找第 ' + aug + ' 条增广路 ———'; });
    yield W(400);
    const path = yield* findPath();
    if (!path) {
      yield S(() => { status.textContent = '残余网络中 s 到 t 已无通路 → 达到最大流！'; });
      yield W(450);
      break;
    }
    ...
    yield S(() => { status.textContent = '找到增广路：' + chain.map(([i]) => NAME[E[i][0]] + '→' + NAME[E[i][1]]).join(' → ') + '，瓶颈容量 ' + bf; });
    yield W(500);
    for (const [i, x] of chain) {
      const [a, b] = E[i];
      setEdgeColor(i, GOLD, 1);
      yield S(() => { status.textContent = '沿 ' + NAME[a] + '→' + NAME[b] + ' 增广 ' + bf + '：flow ' + flow[i] + ' → ' + (flow[i] + path.dir[x] * bf); });
    ...
    yield S(() => { status.textContent = '第 ' + aug + ' 条增广完成，当前总流量 ' + total; });
    yield W(500);
    resetEdgeColors();
  }
  yield S(() => { status.textContent = 'Ford-Fulkerson 演示完成：最大流 ' + total + '，增广 ' + (aug - 1) + ' 次；O(F·E)，容量为整数时终止，值 = 最小割容量'; });
  yield W(600);
  resetEdgeColors();
...
function* runFF() {
  yield S(() => { status.textContent = 'Ford-Fulkerson：DFS 找增广路 + 反向边退流'; });
  yield W(400);
  yield* ffGen();
}
