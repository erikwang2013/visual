// AlgorithmLibrary/KNN3D.js — K 近邻：算距离 → 取最近 K 个 → 多数投票分类
...
const hint = new VText(scene, { text: '点击「运行分类」开始', x: 0, y: 330, z: 0, color: PALETTE.textGlow, scale: 0.85 });
const status = panel.addStatus('');

const K = 3;
const A = [[-220, 90], [-160, 140], [-140, 10], [-260, -30], [-100, -60]];
const B = [[210, 100], [260, 40], [180, -20], [230, -90], [140, -140]];
const Q = [0, 50];
const CLASSES = [
  ...A.map(p => ({ cls: 'A', x: p[0], y: p[1] })),
  ...B.map(p => ({ cls: 'B', x: p[0], y: p[1] })),
];
...
function runKNN() {
  resetAll();
  hint.setText('KNN：计算待分类点与所有已知点的距离，取最近的 K = ' + K + ' 个，按多数投票定类别');
  let done = 0;
  const next = () => {
    if (done >= dists.length) {
    ...
    const d = dists[done]; done++;
    tubes[d.i].visible = true;
    distT[d.i].moveTo(CLASSES[d.i].x, CLASSES[d.i].y - 36, 0, 300);
    distT[d.i].setText('d≈' + d.d.toFixed(1));
    if (done <= K) { pts[d.i].setColor(RED, RED); hint.setText('第 ' + done + ' 近：' + CLASSES[d.i].cls + ' 类（d≈' + d.d.toFixed(1) + '）——进入最近 ' + K + ' 个'); }
    else hint.setText('第 ' + done + ' 近：' + CLASSES[d.i].cls + ' 类（d≈' + d.d.toFixed(1) + '）——距离较远，不参与投票');
    C(650, () => {
      if (done > K) pts[d.i].setColor(CLASSES[d.i].cls === 'A' ? GREEN : BLUE, CLASSES[d.i].cls === 'A' ? GREEN : BLUE);
    });
    C(450, next);
  };
  next();
