...
const hint = new VText(scene, { text: '点击「▶ 演示」开始：K 近邻分类', x: 700, y: 560, z: 0, color: PALETTE.textGlow, scale: 0.7, wrapChars: 7 });
...
const clsT = CLASSES.map((p, i) => new VText(scene, { text: p.cls + '类', x: p.x, y: p.y + 28, z: 0, color: p.cls === 'A' ? GREEN : BLUE, scale: 0.55 }));
const query = new VBox(scene, { w: 34, h: 34, d: 34, x: Q[0], y: Q[1], z: 0, label: '?', color: YELLOW, emissive: YELLOW });
const distT = CLASSES.map(p => new VText(scene, { text: '', x: p.x, y: p.y - 20, z: 0, color: PALETTE.textDim, scale: 0.55 }));
...
function* knnGen() {
  ...
  for (let done = 0; done < dists.length; done++) {
    ...
    yield S(() => {
      ...
      distT[d.i].moveTo(CLASSES[d.i].x, CLASSES[d.i].y - 20, 0, 300);
