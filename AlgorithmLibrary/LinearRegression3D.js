// AlgorithmLibrary/LinearRegression3D.js — 线性回归：梯度下降最小化均方误差，直线逐步拟合数据
...
const hint = new VText(scene, { text: '点击「运行训练」开始', x: 0, y: 330, z: 0, color: PALETTE.textGlow, scale: 0.85 });
...
function clearResiduals() {
  residuals.forEach(t => { t.visible = false; t.geometry.dispose(); });
  residuals = [];
}
function drawResiduals(ww, bb) {
  clearResiduals();
  for (const [x, y] of PTS) {
    residuals.push(tubeBetween(scene, [x, y, 0], [x, ww * x + bb, 0], { color: RED, opacity: 0.45, radius: 1.2 }));
  }
}
...
function runTrain() {
  resetAll();
  hint.setText('线性回归：y = wx + b。用梯度下降反复微调 w、b，让均方误差 MSE 越来越小');
  let done = 0;
  const next = () => {
    ...
    lossT.setText('迭代 ' + (done - 1) * 30 + '：MSE = ' + s.mse.toFixed(1) + '，w = ' + s.w.toFixed(3) + '，b = ' + s.b.toFixed(1));
    eqT.setText('红色竖线 = 残差（预测值 - 真实值），MSE 即残差平方的平均');
    if (done > 1) hint.setText('MSE 从 ' + steps[0].mse.toFixed(0) + ' 降到 ' + s.mse.toFixed(1) + '：直线正逐步贴近数据');
    C(900, next);
  };
  next();
}

panel.addButton('运行训练', runTrain);
