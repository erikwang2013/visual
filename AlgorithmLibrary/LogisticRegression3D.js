// AlgorithmLibrary/LogisticRegression3D.js — 逻辑回归：sigmoid + 梯度下降求决策边界（function* 生成器驱动，3 轮训练动画）
...
const hint = new VText(scene, { text: '点击「运行演示」开始：逻辑回归训练', x: 0, y: 260, z: 0, color: PALETTE.textGlow, scale: 0.85 });
...
function* lrGen() {
  resetAll();
  yield S(() => hint.setText('逻辑回归：p = σ(w₁x + w₂y + b)，σ 把分数压到 0..1，交叉熵作损失'));
  yield S(() => { for (let i = 0; i < PTS.length; i++) ptBoxes[i].setColor(PTS[i][2] ? GREEN : ROSE, PTS[i][2] ? GREEN : ROSE); });
  yield W(300);
  yield S(() => {
  ...
  yield W(900);
  yield S(() => hint.setText('梯度下降：w ← w − lr·∂L/∂w，沿损失下降方向逐步修正边界'));
  for (let k = 0; k < 3; k++) {
    const w = W_ROUNDS[k];
    yield S(() => {
    ...
    yield S(() => {
      ...
      lossT.setText('本轮损失 = ' + LOSS[k].toFixed(3) + '（比上轮' + (k === 0 ? '' : ' ↓') + '，边界向分界处推进）');
    });
    yield W(600);
  }
  yield S(() => {
