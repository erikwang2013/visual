// AlgorithmLibrary/MLP3D.js — MLP 感知机：2-2-1 网络解决 XOR（前向 + 反向传播）
...
const hint = new VText(scene, { text: '点击「训练 MLP」开始', x: 0, y: 255, z: 0, color: PALETTE.textGlow, scale: 0.85 });
...
function runMLP() {
  resetAll();
  hint.setText('MLP：前向算预测，反向算梯度，梯度下降更新权重 — 深度学习的三板斧');
  C(400, () => { stepT.setText('网络：2 输入 → 2 隐藏（σ 激活）→ 1 输出。当前样本 (1,1)，目标 0'); });
  C(900, () => {
    L1.forEach(b => (b.mesh.visible = true));
    h1.setText('h₁ = 0.69'); h2.setText('h₂ = 0.75');
    eq1T.setText('前向①：z₁ = W₁x + b₁ = [0.8, 1.1] → σ 激活 → a₁ = [0.69, 0.75]');
    stepT.setText('输入沿连线传到隐藏层（黄线点亮）');
  });
  C(900, () => {
