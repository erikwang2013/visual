...
new VText(scene, { text: '回文半径立柱：中心红柱，镜像位置柱色向外渐变 红→紫；金球连线 = 对称镜像', x: 700, y: 330, z: 0, color: PALETTE.textDim, scale: 0.5, wrapChars: 12 });
...
function beam(x0, x1) {
  const pts = [new THREE.Vector3(x0, 510, 40), new THREE.Vector3(x1, 510, 40)];
...
function* runManacher() {
  ...
  yield S(() => {
    ...
    ring.mesh.position.set(cx(c), 210, 0); ring.mesh.visible = true;
