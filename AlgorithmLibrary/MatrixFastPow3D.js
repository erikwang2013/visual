...
const hint = new VText(scene, { text: '点击「▶ 演示」开始：矩阵快速幂 M^10', x: 700, y: 560, z: 0, color: PALETTE.textGlow, scale: 0.7, wrapChars: 7 });
const status = panel.addStatus('就绪');
const stageT = new VText(scene, { text: '', x: 700, y: 410, z: 0, color: GOLD, scale: 0.5, wrapChars: 8 });
const eqT = new VText(scene, { text: '', x: 700, y: 280, z: 0, color: PALETTE.textGlow, scale: 0.45, wrapChars: 8 });
const outT = new VText(scene, { text: '', x: 700, y: 280, z: 0, color: PALETTE.textGlow, scale: 0.45, wrapChars: 8 });
...
const bitBoxes = bits.map((b, i) => new VBox(scene, { w: 55, h: 46, d: 46, x: 170 + i * 75, y: 485, z: 0, label: String(b), color: b ? AMBER : DIM, emissive: b ? AMBER : DIM }));
new VText(scene, { text: '指数 10 = 1010₂（左→右低位→高位）', x: 430, y: 490, z: 0, color: PALETTE.textDim, scale: 0.45, wrapChars: 8 });
const mGrid = [[0, 0], [0, 0]].map((row, i) => row.map((v, j) => new VBox(scene, { w: 44, h: 44, d: 44, x: 305 + j * 46, y: 400 - i * 46, z: 0, label: '', color: DIM, emissive: DIM })));
const resGrid = [[0, 0], [0, 0]].map((row, i) => row.map((v, j) => new VBox(scene, { w: 44, h: 44, d: 44, x: 305 + j * 46, y: 275 - i * 46, z: 0, label: '', color: DIM, emissive: DIM })));
new VText(scene, { text: 'M 当前幂', x: 235, y: 400, z: 0, color: VIOLET, scale: 0.45, wrapChars: 8 });
new VText(scene, { text: '结果 res', x: 235, y: 275, z: 0, color: AMBER, scale: 0.45, wrapChars: 8 });
new VText(scene, { text: '矩阵快速幂：M 逐位自乘，位=1 时乘进 res', x: 700, y: 520, z: 0, color: PALETTE.textDim, scale: 0.42, wrapChars: 8 });
new VText(scene, { text: 'M = [[1,1],[1,0]] 是斐波那契「递推矩阵」', x: 700, y: 468, z: 0, color: PALETTE.textDim, scale: 0.42, wrapChars: 8 });
...
function* mfpGen() {
  ...
  yield S(() => { eqT.setText(''); outT.setText('res = ' + fmtS(M10) + ' = M¹⁰ ✓ —— 4 次乘法 vs 朴素 10 次'); status.textContent = '矩阵快速幂：M¹⁰ = [[89,55],[55,34]]（4 次乘法 vs 朴素 10 次）'; });
  yield W(1000);
  yield S(() => stageT.setText('斐波那契彩蛋：M^n = [[F(n+1),F(n)],[F(n),F(n−1)]] —— res[0][1] = 55 = F(10)，res[0][0] = 89 = F(11)'));
  yield W(800);
  yield S(() => outT.setText('应用：斐波那契 O(log n)、马尔可夫链、图论计数；模幂是 RSA 核心'));
  yield W(900);
