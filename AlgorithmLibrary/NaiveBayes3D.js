// AlgorithmLibrary/NaiveBayes3D.js — 朴素贝叶斯：词频计数 + 条件概率分类（垃圾邮件识别）
...
const hint = new VText(scene, { text: '点击「训练分类」开始', x: 0, y: 300, z: 0, color: PALETTE.textGlow, scale: 0.85 });
...
function runTrain() {
  resetAll();
  hint.setText('朴素贝叶斯：P(类|邮件) ∝ P(类)·Π P(词|类) — 垃圾邮件过滤器就是这么算的');
  C(300, () => {
    for (let i = 0; i < MAILS.length; i++) mailBoxes[i].setColor(MAILS[i].spam ? GREEN : ROSE, MAILS[i].spam ? GREEN : ROSE);
