// AlgorithmLibrary/OpenHash3D.js
// 开放哈希（链地址法）：10 个桶 + 桶内链；h(x) = x % 10
...
function insert(value) {
  ...
  C(1, () => {
    ...
    if (chain.length === 0) nullTexts[k].remove();
  }, () => {});
  chain.push(node);
  C(60, () => tmp.remove(), () => {});
  C(60, () => formula.remove(), () => {});
...
function del(value) {
  ...
  if (found < 0) {
    ...
    C(1, () => {
      ...
      if (chain.length === 1) nullTexts[k] = new VText(scene, { text: '/', x: bx(k), y: 0, z: 0, color: PALETTE.textDim, scale: 0.8 });
    }, () => {});
