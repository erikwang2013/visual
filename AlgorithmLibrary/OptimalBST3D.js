// AlgorithmLibrary/OptimalBST3D.js — 最优二叉搜索树：e[i][j]=min(e[i][r-1]+e[r+1][j])+w[i][j] 按区间长填表，根表驱动右侧建树（function* 生成器驱动，解说入状态栏）
...
const BLUE = 0x60a5fa, GOLD = 0xfcd34d, RED = 0xfb7185, ORANGE = 0xfb923c, CYAN = 0x22d3ee, WHITE = 0xffffff;
...
function* obGen() {
  yield S(() => { status.textContent = '最优 BST：高频键不代表要放树顶 —— 深度 × 概率才是代价，必须全局权衡'; });
  yield W(650);
  for (const s of steps) {
    for (let k = s.i; k <= s.j; k++) keys[k - 1].setColor(RED, RED);
    cellOf(s.i, s.j).box.setColor(CYAN, CYAN);
    yield S(() => { status.textContent = '区间 [k' + s.i + '..k' + s.j + ']（长 ' + s.len + '）：w = ' + s.w + '（区间概率和 = 下移一层的附加代价）'; });
    yield W(420);
    for (const cd of s.cands) {
      keys[cd.r - 1].setColor(cd.r === s.br ? ORANGE : CYAN, cd.r === s.br ? ORANGE : CYAN);
      yield S(() => { status.textContent = '候选根 r=' + cd.r + '：e[' + s.i + '][' + (cd.r - 1) + '] + e[' + (cd.r + 1) + '][' + s.j + '] + ' + s.w + ' = ' + cd.t + (cd.r === s.br ? ' ← 当前最优' : ''); });
      yield W(240);
    }
    ...
    yield S(() => { status.textContent = 'e[' + s.i + '][' + s.j + '] = ' + s.cost + '（最优根 r=' + s.br + ' → root[' + s.i + '][' + s.j + '] = ' + s.br + '）'; });
    yield W(480);
  }
  yield S(() => { status.textContent = '表填完：e[1][5] = ' + e[1][5].toFixed(2) + '，root[1][5] = 2 → 根是 k2，按根表递归展开左右子树'; });
  yield W(800);
  for (let idx = 0; idx < TORDER.length; idx++) {
    ...
    yield S(() => { status.textContent = '根表展开：区间 [' + n.lo + '..' + n.hi + '] 的根 = k' + n.key + '（root[' + n.lo + '][' + n.hi + '] = ' + n.key + '）'; });
    yield W(450);
  }
  yield S(() => { status.textContent = '最优树：根 k2，左 k1，右子树根 k5（其左 k4，k4 左 k3）—— 频率最高的 k5(0.20) 在第 2 层'; });
  yield W(750);
  yield S(() => { status.textContent = 'OptimalBST 演示完成：期望查找代价 e[1][5] = ' + e[1][5].toFixed(2) + '（最优根 k2，k5 第 2 层）；O(n³) 时间 / O(n²) 空间，Knuth 优化可降至 O(n²)'; });
  yield W(600);
}
