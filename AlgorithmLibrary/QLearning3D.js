...
const hint = new VText(scene, { text: '点击「▶ 演示」开始：Q-Learning', x: 700, y: 560, z: 0, color: PALETTE.textGlow, scale: 0.7, wrapChars: 7 });
const status = panel.addStatus('就绪');

const SX = [80, 240, 400, 560];
const states = SX.map((x, i) => new VBox(scene, { w: 80, h: 80, d: 80, x, y: 390, z: 0, label: 's' + i, color: i === 3 ? GREEN : DIM, emissive: i === 3 ? GREEN : 0 }));
new VText(scene, { text: '终点奖励 +10', x: 560, y: 490, z: 0, color: GREEN, scale: 0.6 });
new VText(scene, { text: '4 格走廊：从 s₀ 出发，到达 s₃ 得 +10', x: 280, y: 490, z: 0, color: PALETTE.textDim, scale: 0.6 });
...
const rightArrows = [0, 1, 2].map(i => arrow(SX[i] + 40, SX[i + 1] - 40, 422, BLUE));
const leftArrow = arrow(SX[3] - 40, SX[2] + 40, 358, ROSE);

// 智能体（黄块）
const agent = new VBox(scene, { w: 36, h: 36, d: 36, x: SX[0], y: 390, z: 0, label: 'agent', color: YELLOW, emissive: YELLOW });

// Q 表显示（每状态一行）
const qT = SX.map((x, i) => new VText(scene, { text: 'Q(s' + i + ') = [0, 0]', x, y: 270, z: 0, color: PALETTE.textGlow, scale: 0.6 }));
new VText(scene, { text: 'Q(s,a) ← Q(s,a) + lr·(R + γ·maxQ(s′) − Q(s,a))    lr = 0.5，γ = 0.9', x: 0, y: 190, z: 0, color: PALETTE.textDim, scale: 0.6 });

const stepT = new VText(scene, { text: '', x: 0, y: 125, z: 0, color: PALETTE.textGlow, scale: 0.75 });
...
function resetAll() {
  ...
  agent.mesh.position.set(SX[0], 390, 0);
...
function* qGen() {
  ...
  for (const i of [0, 1, 2]) {
    ...
    yield A(450, p => agent.mesh.position.set(SX[i] + (SX[i + 1] - SX[i]) * p, 390, 0));
    yield S(() => {
      agentAt(i + 1);
