...
const hint = new VText(scene, { text: '点击「运行演示」开始：QR 分解 A = Q·R', x: 0, y: 300, z: 0, color: PALETTE.textGlow, scale: 0.85 });
...
function* qrGen() {
  yield S(() => { hint.setText('把 A 拆成 Q·R：第 1 列直接归一化；之后每列先减掉对已有正交基的投影（玫瑰），再归一化（金色）—— 逐列拼出正交基'); stageT.setText('Gram–Schmidt：先「投影减法」剥离旧方向，再「归一化」成单位向量'); });
  yield W(700);
  for (let j = 0; j < colSteps.length; j++) {
    const cs = colSteps[j];
    let uBoxesArr = null;
    for (let i = 0; i < N; i++) aCells[i][j].setColor(CYAN, CYAN);
    yield S(() => stageT.setText('第 ' + (j + 1) + ' 列：取出 c' + (j + 1) + ' = (' + cs.cj.map(f2).join(', ') + ')' + (j > 0 ? ' —— 准备剥离对 q1…q' + j + ' 的投影' : ' —— 第一列没有旧基可投影')));
    yield W(620);
    yield S(() => hint.setText(j > 0 ? '新列要与所有旧基正交：u = cj − r1j·q1 − r2j·q2 − …，把「重叠的部分」全部剥掉' : '第一列当方向直接归一化 —— 正交基的第一个成员诞生'));
    yield W(300);
    uBoxesArr = cs.cj.map((v, i) => addTemp(() => new VBox(scene, { w: 36, h: 36, d: 36, x: AX(i), y: 15, z: 0, label: String(f2(v)), color: DIM, emissive: DIM })));
    ...
    for (let k = 0; k < cs.proj.length; k++) {
      ...
      yield S(() => stageT.setText('投影系数 r' + (k + 1) + (j + 1) + ' = q' + (k + 1) + '·c' + (j + 1) + ' = ' + rk + ' —— 写进 R 第 ' + (k + 1) + ' 行'));
      yield W(700);
      yield S(() => hint.setText('点积 = 对应分量相乘再求和，度量「c 在 q 方向上的分量」—— 这部分要全部减掉'));
      yield W(300);
      ...
      yield S(() => stageT.setText('投影减法：u ← u − r' + (k + 1) + (j + 1) + '·q' + (k + 1) + ' —— 玫瑰色 = 刚被改动的分量'));
      yield W(650);
    ...
    yield S(() => stageT.setText('归一化：r' + (j + 1) + (j + 1) + ' = ‖u‖ = ' + f2(cs.norm) + '，q' + (j + 1) + ' = u / ‖u‖ = (' + cs.qj.map(f2).join(', ') + ') —— 填入 Q 第 ' + (j + 1) + ' 列'));
    yield W(800);
    yield S(() => hint.setText('归一化 = 除以模长把长度变成 1 —— 验证：‖q‖² = 各分量平方和 = 1'));
    yield W(300);
  ...
  yield S(() => { outT.setText('正交检验：q1·q2 = ' + q1q2 + ' ≈ 0，‖q1‖ = ‖q2‖ = ‖q3‖ = 1 —— QᵀQ = I，A = Q·R 精确还原'); status.textContent = 'QR 分解：Q = [[0.71,0.41,-0.58],[0.71,-0.41,0.58],[0,0.82,0.58]]，R = [[1.41,0.71,0.71],[0,1.22,0.41],[0,0,1.15]]'; });
  yield W(1000);
  yield S(() => { hint.setText('为什么用 QR？最小二乘 Ax≈b：Qᵀ 作用后变成 R·x = Qᵀb —— 上三角一次回代，正交性保证数值稳定'); outT.setText('复杂度 O(n³)：每列投影 O(n²)。应用：最小二乘、QR 迭代求特征值、Householder/Givens 反射是其数值变体'); });
  yield W(1200);
...
panel.addButton('运行演示', () => engine.start(runQR()));
