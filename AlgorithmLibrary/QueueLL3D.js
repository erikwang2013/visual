...
const scene = new Scene3D('scene', { cameraPos: [278, 480, 1000], lookAt: [278, 100, 0], fov: 52 });
const engine = new GeneratorEngine({ speed: 1 });
const panel = new ControlPanel({ engine });

const BLUE = 0x60a5fa, GOLD = 0xfcd34d, GREEN = 0x4ade80, RED = 0xfb7185, ORANGE = 0xfb923c, CYAN = 0x22d3ee, PUR = 0xc4b5fd, WHITE = 0xffffff, DIM = 0x334155;
const hint = new VText(scene, { text: '点击「▶ 演示」开始：链表队列 入队×6 + 出队×3（Head/Tail 双指针）', x: 760, y: 460, z: 0, color: PALETTE.textGlow, scale: 0.7, wrapChars: 7 });
...
const headLbl = new VText(scene, { text: 'Head →', x: NODE_X(0) - 90, y: 148, z: 0, color: CYAN, scale: 0.55 });
const tailLbl = new VText(scene, { text: 'Tail →', x: NODE_X(-1) + 100, y: 28, z: 0, color: GOLD, scale: 0.55 });
const emptyT = new VText(scene, { text: '空队列', x: 120, y: 60, z: 0, color: PALETTE.textDim, scale: 0.6 });
...
function relayout() {
  ...
  headLbl.moveTo(nodes.length ? NODE_X(0) - 90 : 120, 148, 0, 380);
  tailLbl.moveTo(nodes.length ? NODE_X(nodes.length - 1) + 100 : 120, 28, 0, 380);
