...
const hint = new VText(scene, { text: '点击「运行演示」开始：SHA-256 —— 8 寄存器 + 64 轮压缩，输出 256 位摘要', x: 0, y: 300, z: 0, color: PALETTE.textGlow, scale: 0.85 });
...
new VText(scene, { text: 'W[r]：W[0..15] = 分组 16 字；W[16..63] = σ0(W[r−15]) + W[r−7] + σ1(W[r−2]) + W[r−16]', x: 0, y: -45, z: 0, color: PALETTE.textDim, scale: 0.36 });
...
panel.addButton('运行演示', () => engine.start(runSHA256()));
panel.addButton('清空', () => {
  engine.clear();
