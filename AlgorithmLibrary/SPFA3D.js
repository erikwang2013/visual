...
function* pushBox(id) {
  ...
  const box = new VBox(scene, { w: 42, h: 42, d: 20, x, y: 420, z: 0, label: id, color: ORANGE, emissive: ORANGE });
...
function* spfaGen() {
  ...
  const q = [0];
  inQ[0] = true;
  yield S(() => { status.textContent = '初始化：dist[0]=0，0 入队。SPFA 只松弛出队节点的出边'; });
  ...
  while (head < q.length) {
    ...
    inQ[u] = false;
    setNodeColor(u, GOLD);
    yield S(() => { status.textContent = '出队 ' + u + '：松弛它的全部出边'; });
    yield* popBox();
    yield W(380);
    for (const [v, w] of adj[u]) {
      ...
      if (cand < dist[v]) {
        ...
        if (!inQ[v]) {
          ...
          q.push(v);
          cnt[v]++;
          yield S(() => { status.textContent = v + ' 不在队列 → 入队（cnt[' + v + ']=' + cnt[v] + '）'; });
        ...
        yield S(() => { status.textContent = '边 ' + u + '→' + v + '：dist 不更新'; });
        yield W(220);
      }
      resetEdgeColors();
    }
  ...
  if (negCycle) {
    yield S(() => { status.textContent = '检测到负环：节点入队次数 ≥ V，SPFA 演示完成'; });
  } else {
    yield S(() => { status.textContent = '队列清空：所有节点达到最终距离'; });
    yield W(400);
    setNodeColor(4, GOLD);
    yield S(() => { status.textContent = 'SPFA 演示完成：0→4 = ' + dist[4] + '，无负环，平均 O(E)'; });
  }
  yield W(500);
  resetEdgeColors();
