...
const scene = new Scene3D('scene', { cameraPos: [316, 460, 900], lookAt: [316, 72, 0], fov: 52 });
const engine = new GeneratorEngine({ speed: 1 });
const panel = new ControlPanel({ engine });

const BLUE = 0x60a5fa, GOLD = 0xfcd34d, GREEN = 0x4ade80, RED = 0xfb7185, ORANGE = 0xfb923c, CYAN = 0x22d3ee, PUR = 0xc4b5fd, WHITE = 0xffffff, DIM = 0x334155;
const hint = new VText(scene, { text: '点击「▶ 演示」开始：下标式数组栈 push×7 + pop×4（top 框右移左移）', x: 760, y: 440, z: 0, color: PALETTE.textGlow, scale: 0.7, wrapChars: 7 });
const status = panel.addStatus('就绪');
const stageT = new VText(scene, { text: '', x: 760, y: 340, z: 0, color: GOLD, scale: 0.55, wrapChars: 8 });
const eqT = new VText(scene, { text: '', x: 760, y: 250, z: 0, color: PALETTE.textGlow, scale: 0.55, wrapChars: 8 });
const outT = new VText(scene, { text: '', x: 760, y: 160, z: 0, color: PALETTE.textGlow, scale: 0.53, wrapChars: 8 });

const SIZE = 12;
const slotX = i => 345 + (i - (SIZE - 1) / 2) * 48;
const slots = [];
for (let i = 0; i < SIZE; i++) slots.push(new VBox(scene, { w: 40, h: 46, d: 46, x: slotX(i), y: 53, z: 0, label: '', color: DIM, emissive: DIM }));
const idxT = [];
for (let i = 0; i < SIZE; i++) idxT.push(new VText(scene, { text: String(i), x: slotX(i), y: 1, z: 0, color: PALETTE.textDim, scale: 0.4 }));
const topBox = new VBox(scene, { w: 58, h: 20, d: 20, x: slotX(-1), y: 115, z: 0, label: 'top=空', color: GOLD, emissive: GOLD });
const topArr = new VText(scene, { text: '▼', x: slotX(-1), y: 97, z: 0, color: GOLD, scale: 0.5 });
...
function moveTop() {
  const x = slotX(top);
  topBox.moveTo(x, 115, 0, 350);
  topArr.moveTo(x, 97, 0, 350);
