// AlgorithmLibrary/Strassen3D.js — Strassen 矩阵乘法：7 个乘法代替 8 个 —— 用加法换乘法，递归地把 O(n³) 压到 O(n^2.807)
...
const hint = new VText(scene, { text: '点击「运行 Strassen 乘法」开始', x: 0, y: 300, z: 0, color: PALETTE.textGlow, scale: 0.85 });
...
const aCells = A.map((row, i) => row.map((v, j) => cell(v, AX(j), ARY(i))));
...
function resetAll() {
  engine.clear();
  A.forEach((row, i) => row.forEach((v, j) => { setCell(aCells[i][j], v, DIM); }));
...
function runStrassen() {
  resetAll();
  hint.setText('朴素算法每个 C 块要 2 次乘法，共 8 次 —— Strassen 的戏法：把 a 和 b 的组合预先打包，7 次乘法足矣');
  C(600, () => {
    for (let i = 0; i < 2; i++) for (let j = 0; j < 2; j++) { aCells[i][j].setColor(CYAN, CYAN); bCells[i][j].setColor(CYAN, CYAN); }
    stageT.setText('目标：C = A×B。先看朴素做法 —— 每次乘法都「锁死」一个 a 和一个 b，8 次');
    hint.setText('Strassen 的洞察：矩阵元素可以「预组合」—— 组合后的中间量比原始元素更能复用信息');
  });
  P.forEach((p, t) => {
    C(650, () => {
      ...
      stageT.setText(`${p.name}：先对 A、B 的元素做加减法（青色），得到两个组合数 —— 一个乘法还没开始`);
      hint.setText('加减法不要钱，乘法才是瓶颈 —— 所以 P 的构造里全是加减，只有最后的 × 计入成本');
    });
    C(600, () => {
      pCards[t].box.setColor(GOLD, GOLD);
      pCards[t].valT.setText(`= ${p.v}`, { color: GOLD });
      eqT.setText(`${p.name} = ${p.num}`, { color: GOLD });
      stageT.setText(`${p.name} = ${p.num} —— 一次乘法完成（卡变金色）`);
    });
    C(300, () => {
  ...
  C(500, () => {
    ...
    stageT.setText('7 张 P 卡全部就位 —— 现在用「加减组合」把 C 的 4 块拼出来，零乘法');
  });
  Cm.forEach((c, t) => {
    C(700, () => {
      c.uses.forEach(u => { pCards[u].box.setColor(ROSE, ROSE); });
      eqT.setText(`C[${c.i + 1}][${c.j + 1}] = ${c.expr}`, { color: ROSE });
      stageT.setText(`拼 C[${c.i + 1}][${c.j + 1}]：${c.expr} —— 玫瑰色 = 参与的 P 卡`);
    });
    C(600, () => {
      setCell(cCells[t], c.v, GOLD);
      eqT.setText(`C[${c.i + 1}][${c.j + 1}] = ${c.num} = ${c.v}`, { color: GOLD });
      stageT.setText(`C[${c.i + 1}][${c.j + 1}] = ${c.v} 落定（金色）—— 只加不加，一次乘法都没用`);
    });
  });
  C(1000, () => {
    for (let i = 0; i < 2; i++) for (let j = 0; j < 2; j++) { aCells[i][j].setColor(CYAN, CYAN); bCells[i][j].setColor(CYAN, CYAN); }
    pCards.forEach(p => p.box.setColor(VIOLET, VIOLET));
    outT.setText(`C = [[${Cm[0].v},${Cm[1].v}],[${Cm[2].v},${Cm[3].v}]] ✓ 与朴素一致 —— 8 次乘法减到 7 次：T(n) = 7T(n/2) + O(n²) → O(n^2.807)`);
    status.textContent = `Strassen：7 次乘法得 C = [[19,22],[43,50]]（朴素 8 次，结果一致）`;
    hint.setText('递归的威力：n×n 切成 4 个 n/2 块，每层都省 1/8 的乘法 —— 反复套用，省出来的份额越滚越大');
  });
  C(1200, () => {
    outT.setText('应用：大规模矩阵乘（BLAS/深度学习）、Winograd 变体进 SIMD 库 —— 常数大，n 小时朴素反而快，工程上约 n>64 切换');
    hint.setText('更激进：Coppersmith–Winograd 理论上 O(n^2.37)，但常数天文数字 —— 工程主流就是 Strassen 及其变体');
  });
}

panel.addButton('运行 Strassen 乘法', runStrassen);
