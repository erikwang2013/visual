...
const hint = new VText(scene, { text: '点击「运行演示」开始：Tarjan 强连通分量（dfn/low）', x: 0, y: 315, z: 0, color: PALETTE.textGlow, scale: 0.85 });
...
function* popSCC(u, color) {
  ...
  do {
    ...
    yield A(220, p => { e.box.mesh.scale.setScalar(1 - p); });
    scene.remove(e.box.mesh);
  } while (String(e.id) !== String(u));
  for (const m of members) setNodeColor(m, color);
  sccList.push(members);
  yield S(() => outT.setText('发现 SCC {' + members.join(',') + '}（dfn=low 的根节点弹出）'));
  yield W(600);
}
...
function* tarjan(u) {
  ...
  yield S(() => outT.setText('访问 ' + u + '：dfn=low=' + timer + '，压栈'));
  yield W(420);
  for (const v of adj[u]) {
    if (state[v] === 0) {
      ...
      showDL(u);
      yield S(() => outT.setText('回边 ' + u + '→' + v + '（' + v + ' 在栈中）：low[' + u + '] → ' + low[u]));
      yield W(380);
      resetEdgeColors();
    } else {
      setEdgeColor(u, v, PUR, 0.8);
      yield S(() => outT.setText('横叉边 ' + u + '→' + v + '（' + v + ' 已出栈归属别的 SCC），忽略'));
      yield W(280);
      resetEdgeColors();
    }
  }
  if (low[u] === dfn[u]) {
    yield S(() => outT.setText(u + '：dfn=' + dfn[u] + ' === low=' + low[u] + ' → 是 SCC 根，弹出栈顶到 ' + u));
...
function* tarjanGen() {
  ...
  yield S(() => outT.setText('Tarjan：dfn=首次访问序号，low=可达祖先最小 dfn。dfn==low 即 SCC 根'));
  yield W(600);
  for (let i = 0; i < N; i++) {
    if (state[i] === 0) {
  ...
  yield W(550);
  yield S(() => {
    status.textContent = 'Tarjan 完成：' + sccList.length + ' 个 SCC，O(V+E)';
  });
  yield W(450);
  resetEdgeColors();
}
...
function* runTarjan() {
  buildGraph();
  hint.setText('Tarjan：dfn/low 判 SCC 根，一次 DFS 完成');
  yield W(400);
  yield* tarjanGen();
  yield S(() => { outT.setText(''); hint.setText('Tarjan 完成：SCC ' + sccList.length + ' 个：' + sccList.map(s => '{' + s.join(',') + '}').join(' ')); });
}

panel.addButton('运行演示', () => engine.start(runTarjan()));
panel.addButton('清空', () => { engine.clear(); clearView(); hint.setText('已清空，可重新运行'); status.textContent = ''; outT.setText(''); });
panel.addLabel('（拖拽旋转视角，滚轮缩放；青 = 树边，红 = 回边，紫 = 横叉边；同色节点 = 同一 SCC；顶行为栈）');
