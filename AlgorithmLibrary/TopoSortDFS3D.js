...
function* dfs(u) {
  ...
  yield S(() => { status.textContent = '访问 ' + u + '（置灰，压栈），探索出边'; });
  yield W(420);
  for (const v of adj[u]) {
    if (state[v] === 0) {
      setEdgeColor(u, v, CYAN, 1);
      yield S(() => { status.textContent = '树边 ' + u + '→' + v + '：递归访问 ' + v; });
      yield W(350);
      yield* dfs(v);
      resetEdgeColors();
    } else if (state[v] === 1) {
      setEdgeColor(u, v, RED, 1);
      yield S(() => { status.textContent = '回边 ' + u + '→' + v + '（' + v + ' 仍在栈中灰）→ 存在环，无法拓扑排序！'; });
      yield W(500);
      resetEdgeColors();
    } else {
      setEdgeColor(u, v, ORANGE, 0.8);
      yield S(() => { status.textContent = '跨边 ' + u + '→' + v + '（' + v + ' 已完成），跳过'; });
      yield W(280);
      resetEdgeColors();
    }
  }
  ...
  yield* popBox();
  order.unshift(u);
  yield S(() => { status.textContent = u + ' 完成（后序）→ 压入序列头部。当前拓扑序：' + order.join(' → '); });
  yield W(450);
}

function* topoGen() {
  state = Array(N).fill(0);
  yield S(() => { status.textContent = 'DFS 后序：节点完成顺序的逆序 = 拓扑序。0=白未访 1=灰在栈 2=黑完成'; });
  yield W(550);
  yield* dfs(0);
  yield S(() => { status.textContent = '从 0 可达的全部节点已完成'; });
  yield W(400);
  yield S(() => { status.textContent = '拓扑排序(DFS) 演示完成：序列 ' + order.join(' → ') + '，无环，O(V+E)'; });
  yield W(600);
}
