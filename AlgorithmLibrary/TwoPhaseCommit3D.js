...
function* tpcGen() {
  ...
  yield W(900);
  yield S(() => { parts.forEach((p, i) => { p.setColor(GREEN, GREEN); pStates[i].setText('已就绪 ✓'); }); status.textContent = '3 个参与者本地执行成功、资源锁定 → 回复 YES（3/3），等待最终指令'; });
  yield W(900);
  ...
  yield W(900);
  yield S(() => { parts.forEach((p, i) => { p.pulse(0.4); pStates[i].setText('已提交 ✓'); }); status.textContent = '3 个参与者全部提交成功、释放资源 —— 扣款与入账同时生效'; });
  yield W(900);
